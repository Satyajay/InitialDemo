...
class Delivered extends Component {
                ...
                if (result.status == 1) {
                    ...
                    for (i = 0; i < result.data.length; i++) {
                        ...
                        let obj = {
                            ...
                            time: deliv.time,
                            expireTime: deliv.orderExpireTime,
                            date: new_date,
                            maxTime: new_max,
                            minTime: new_min,
    ...
    //     let dely = await nextProps.deliveredInfo;
    //     if (dely != null) {
    //         let delyArr = []
    //         for (i = 0; i < dely.length; i++) {
    //             var deliv = dely[i];
    //             let new_min = moment.utc(deliv.minTime).format('HH:mm A');
    //             let new_max = moment.utc(deliv.maxTime).format('HH:mm A');
    //             let new_date = moment.utc(deliv.updatedAt, "YYYY-MM-DDTHH:mm:ss Z").format('DD/MM/YYYY');
                    ...
                    <Content>
                        ...
                        <List>
                            <FlatList
                                ...
                                renderItem={({ item }) =>
                                    <TouchableOpacity onPress={() => this.delivered_details(item)}>
                                        <Card>
                                            <CardItem>
...
const mapStateToProps = state => ({
    ...
    deliveredInfo: state.user.deliveredData
});
...
export default connect(mapStateToProps, mapDispatchToProps)(withNavigation(Delivered));