      ...
      if(imageType=='gallery')
{
  ...
  this.setState({avatarSource: source,galleryImages:{uri: image.path, 
    name: name,
     filename: name,
     type: image.mime}},()=>{
...
arrImg && this.saveGalleryImages();
  ...
  saveBannerImage(){
    ...
    //   body.append("banerimage",this.state.bannerImage)
          body.append("custbannerPic",{uri: element.uri, name: element.fileName, filename: element.fileName, type: element.type})
	
        }
console.log(this.state.bannerImage);

      RestClient.imageUpload("drivers/customerinfoupdate",body,tokenValue).then((result) => {
   ...
   />
          ...
          {/* <View style={{ marginHorizontal: 20 }}> */}
            ...
            {/* {this.state.data.data && this.state.data.data.length>0 && this.state.data.data[0].role=="DRIVER"&& <View style={{ flexDirection: "row"}}>
              ...
              >
                <Text onPress={() => this.props.navigation.dispatch(HomeresetAction)}
                  style={[
                    styles.reviewText,
