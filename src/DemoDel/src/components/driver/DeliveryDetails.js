...
that.setState({range:that.getDistanceFromLatLonInKm(that.props.locationData.nextPickUpLocation[0].end_location.lat
...
//    alert(pick)
         ...
         { <View style={[{flex : 1}]}>
           <SubmitButton
             onPress={() =>  {
              ...
              this.props.UserActions.acceptOrder({orderId:this.state.orderId,
              driverId:this.props.userData.data._id,lat_long:lat_long}).then((response) => {
 ...
 <SubmitButton
             onPress={() =>  {
              ...
              this.props.UserActions.rejectOrder({orderId:this.props.orderstate.OrderData.placedOrderData.orderId,
  ...
  ScreenNo2()
  {
    ...
    return(
      <View style={[{flexDirection:'row',marginBottom:10}]}>
         {  <View style={[{flex : 1}]}>
      <SubmitButton
        onPress={() => this.props.UserActions.scheduledOrder({orderId:this.state.orderId,
...
/>
  ...
  changeRoute=()=>{
    ...
    this.setState({ range:that.props.locationData.nextPickUpLocation!=null?this.convertInMeter(that.props.locationData.nextPickUpLocation[0].distance.text):0,
  ...
  markPickUpArrived=()=>{
    ...
    if(this.state.ButtonText=='DROP OFF'){
      this.props.UserActions.dropDone({dropupId:this.props.locationData.nextPickUpLocation[0]._id,
        orderId:this.props.locationData.nextPickUpLocation[0].order_id}).then((response) => {
        if(response.ok==1)
    ...
    {
      this.props.UserActions.dropArrive({dropupId:this.props.locationData.nextPickUpLocation[0]._id,
      orderId:this.props.locationData.nextPickUpLocation[0].order_id}).then((response) => {
     if(response.ok==1)
