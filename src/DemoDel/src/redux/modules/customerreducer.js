...
const initialState = {
  ...
  vehicleID: 0,
  vehicleName: 'bike',
  pickupArr: [{ id: 0, address: 'Choose Pickup Location', img: 'plus', next: 3, prev: 1, lat: 0, long: 0 }],
  dropArr: [{ id: 0, address: 'Choose Drop Off Location', img: 'plus', next: 3, prev: 1, lat: 0, long: 0 }],

  //pickupArr:[{id:0,address : '228 Park Ave S, New York, NY 10003, USA',img : 'plus',next:3,prev:1,lat:0,long:0},
  //            {id:1,address : '228 Park Ave S, New York, NY 10003, USA',img : 'plus',next:3,prev:1,lat:0,long:0},
  //            {id:2,address : 'Choose Pickup Location',img : 'plus',next:3,prev:1,lat:0,long:0}],
  //dropArr:[{id:0,address : '228 Park Ave S, New York, NY 10003, USA',img : 'plus',next:3,prev:1,lat:0,long:0}],
  //          {id:1,address : 'Choose Drop Off Location',img : 'plus',next:3,prev:1,lat:0,long:0}],
  ...
  TabHeaderImages: [
    { tag: 0, displayimg: Constants.Images.customer.locationmenu, img: Constants.Images.customer.locationmenu, activeimg: Constants.Images.customer.locationmenu_active, color: "#1E4281" },
    { tag: 1, displayimg: Constants.Images.customer.servicesmenu, img: Constants.Images.customer.servicesmenu, activeimg: Constants.Images.customer.servicesmenu_active, color: "#1E4281" },
  ...
  //   products:[
  //     {category:1,id:1,name:'Television',desc:'TV, LED, LCD, Plasma ect',quantity:0,img:Constants.Images.customer.television,qtyInCircle:0},
  //     {category:1,id:2,name:'Fridge',desc:'Double Door',quantity:0,img:Constants.Images.customer.fridge,qtyInCircle:0},
  //     {category:1,id:3,name:'Washing',desc:'Front Open door',quantity:0,img:Constants.Images.customer.washing,qtyInCircle:0},
  //       ],
  //     },
  //     {
  //       category:2,
  ...
  //       products:[
  //         {category:2,id:1,name:'King Bed',desc:'Double Bed',quantity:0,img:Constants.Images.customer.bed,qtyInCircle:0},
  //         {category:2,id:2,name:'Cupboard',desc:'Cupboard',quantity:0,img:Constants.Images.customer.cupboard,qtyInCircle:0},
  //         {category:2,id:3,name:'Bedside_Table',desc:'Table',quantity:0,img:Constants.Images.customer.bedside_table,qtyInCircle:0},
  //       ],
  //     },
  //     {
  //       category:3,
  ...
  //       products:[
  //         {category:3,id:1,name:'Sofa',desc:'Sofa',quantity:0,img:Constants.Images.customer.sofa,qtyInCircle:0},
  //         {category:3,id:2,name:'Dinner Table',desc:'Table',quantity:0,img:Constants.Images.customer.dinner_table,qtyInCircle:0},
  //         {category:3,id:3,name:'Shelf',desc:'Shelf',quantity:0,img:Constants.Images.customer.shelf,qtyInCircle:0}
  //       ],
  //     },
  //   ],
...
export default function CustomerReducer(state = initialState, action = {}) {
  ...
  switch (action.type) {
    ...
    case 'IS_DELIVERYDATE':
      ...
      if (newstate.vehicleID > 0 && newstate.IsStartTime) {
    ...
    case 'IS_STARTUPTIME':
      ...
      if (newstate.vehicleID > 0 && newstate.IsDeliveryDate) {
        newstate.ActiveNextBackColor = '#53C8E5';
        newstate.ActiveNextTextColor = '#FFFFFF';
        newstate.ActiveButton = true;
      }
    ...
    case 'ADD_PICKUP':
      {
        ...
        //   arrMarker.push({ id: 0, pickdropflag: newstate.PickDropFlag, pickdropId: len - 1, title: 'Pickup ' + (len), coordinates: { latitude: action.lat, longitude: action.long, latitudeDelta: 1, longitudeDelta: 1 }, img: Constants.Images.customer.markerBlue });
        // }
        // else {
    ...
    case 'ADD_DROP':
      ...
      if (len < 3 && newstate.pickupArr.length < 3) {
        arr1.push({ id: len, address: 'Choose Drop Off Another Location', img: 'plus', next: 3, prev: 1, lat: 0, long: 0 });
        newstate.pickUpControlCount = newstate.pickUpControlCount + 0.85;
      }
    ...
    case 'SET_DURATION':
      ...
      //newstate.HourlyServiceDuration = action.
      if (newstate.HourlyServiceCount == 3) {
        newstate.EstimateColor = '#53C8E5';
        newstate.EstimateButtonBackgroundColor = '#ffffff';
        newstate.DeliveryServiceOpacity = 1;
    ...
    case 'SET_VEHICLECOST':
      var transportArr1 = action._data.vehicle;
      console.log(action._data.vehicle);
      ...
      transportArr1.map((val, i) => {
        ...
        //    if(parseInt(val.tag)==transportArr1[i].id)
        {
          arr1.push({
            ...
            activeimgsource: CustomerConnection.mediaURL() + val.imagePath, header: val.name,
    ...
    case 'ADD_PICKUP_HOURLY':
      ...
      arr1.push({ id: len - 1, address: action.pickup, img: 'none', lat: action.lat, long: action.long });

      //arr1.push({id:len,address : 'Choose Another Pickup Location',img : 'plus',lat:0,long:0});
      ...
      newstate.markerPositions.map((val, i) => {
        if ((val.title.indexOf('Drop') == 0))
          arrMarker.push({ id: val.id, pickdropflag: val.pickdropflag, pickdropId: val.pickdropId, title: val.title, coordinates: val.coordinates, img: val.img });
      });
      arrMarker.push({ id: 0, pickdropflag: newstate.PickDropFlag, pickdropId: len - 1, title: 'Pickup', coordinates: { latitude: action.lat, longitude: action.long, latitudeDelta: 1, longitudeDelta: 1 }, img: Constants.Images.customer.markerBlue });
    ...
    case 'ADD_DROP_HOURLY':
      ...
      arr1.push({ id: len - 1, address: action.drop, img: 'none', lat: action.lat, long: action.long });

      //arr1.push({id:len,address : 'Choose Drop Off Another Location',img : 'plus',lat:0,long:0});
      ...
      newstate.markerPositions.map((val, i) => {
        if ((val.title.indexOf('Pickup') == 0))
          arrMarker.push({ id: val.id, pickdropflag: val.pickdropflag, pickdropId: val.pickdropId, title: val.title, coordinates: val.coordinates, img: val.img });
      });

      arrMarker.push({ id: 1, pickdropflag: newstate.PickDropFlag, pickdropId: len - 1, title: 'Drop', coordinates: { latitude: action.lat, longitude: action.long, latitudeDelta: 1, longitudeDelta: 1 }, img: Constants.Images.customer.markerOrange });
      ...
      newstate.DisplayLocationAddress.map((val, b) => {
        if (parseInt(newstate.CourierItemIndex) == parseInt(val.id)) {
          for (var i = 0; i < val.courieritems.length; i++) {
            _courieritems.push({
              parentId: newstate.CourierItemIndex, id: val.courieritems[i].id, name: val.courieritems[i].name,
              size: val.courieritems[i].size.toFixed(2), weight: val.courieritems[i].weight, quantity: val.courieritems[i].quantity,
          ...
          _courieritems.push({ parentId: newstate.CourierItemIndex, id: len, name: name, size: _size.toFixed(2), weight: _weight, quantity: 1, height: action.height, width: action.width, depth: action.depth, isSkid: action.isSkid });
