...
class RestClient {
    ...
    static post(url, params, token = '', userId = '') {
        ...
        return new Promise(function (fulfill, reject) {
            context.isConnected().then(() => {
                ...
                fetch(Connection.getCustomerTempUrl() + url, {
                    method: "POST",
                    timeout: 1000 * 1 * 60,
                    headers: {
    ...
    static urlPost(url, params, token = '', userId = '') {
        let context = this,
            logintoken;
        return new Promise(function (fulfill, reject) {
            context.isConnected().then(() => {
    ...
    static urlAdmin(url, params, token = '', userId = '') {
        let context = this,
            logintoken;
        return new Promise(function (fulfill, reject) {
            context.isConnected().then(() => {
    ...
    static urlPostFile(url, params) {
        ...
        return new Promise(function (fulfill, reject) {
            context.isConnected().then(() => {
                ...
                data.append('order_id', params.order_id);
                data.append('order_type', params.order_type);
                data.append('location', params.location);
                data.append('locationimage', params.locationimage);
                fetch(Connection.getTempUrl() + url, {
                    method: "POST",
                    timeout: 1000 * 1 * 60,
                    headers: {
    ...
    static get_New(url, params) {
        ...
        return new Promise(function (fulfill, reject) {
            context
                ...
                .then(() => {
                    let query = querystring.stringify(params);
                    fetch(Connection.getTempUrl() + url + "?" + query, {
                        method: "GET",
                        timeout: 1000 * 1 * 60,
                        headers: {
                            "Accept": "application/json",
