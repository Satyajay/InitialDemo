...
export default class ResetPin extends Component {
  ...
  resetPass() {
    ...
    if (old_pass == '' || old_pass == null) {
      this.callToast('Please enter reset code', 'error')
      ...
      AsyncStorage.getItem('auth_key').then((Auth_key) => {
        ...
        var data = {
          user_id: Auth_key,
          reset_code: this.state.old_pass,
          secure_pin: this.state.new_pass
        };
        console.log(data)
        this.setState({ animating: true });
        Apis.postAPI('wsupdatepin', data).then(response => {
  ...
  render() {
    ...
    return (
      <View style={styles.container}>
        <Header onPress={() => this.props.navigation.goBack()} HeaderText='Reset PIN' />
        <Modal
          transparent={true}
          onRequestClose={() => null}
        ...
        <Modal
          ...
          <View style={styles.transaparentView}>
            <View style={styles.centerView}>
              ...
              <Text style={styles.desc}>Your PIN is changed {'\n'} successfully.</Text>
            </View>
          </View>
        </Modal>
      </View>
